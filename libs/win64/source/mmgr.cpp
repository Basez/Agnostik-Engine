// Please note: I (https://github.com/Basez) Made some changes to this file to make it compatible for MSVC14!
...
static		unsigned int	s_currentAllocationCount = 0;
static		unsigned int	s_breakOnAllocationCount = 0;
static		sMStats		s_stats;
static	const	char		*s_sourceFile            = "??";
static	const	char		*s_sourceFunc            = "??";
static		unsigned int	s_sourceLine             = 0;
static		bool		s_staticDeinitTime       = false;
...
static	void	doCleanupLogOnFirstRun()
{
	if (cleanupLogOnFirstRun)
	{
		...
		log("      %s - Memory logging file created", memoryLogFile);
		log("--------------------------------------------------------------------------------");
		log("");
...
static	const char	*sourceFileStripper(const char *a_sourceFile)
{
	const char	*ptr = strrchr(a_sourceFile, '\\');
	if (ptr) return ptr + 1;
	ptr = strrchr(a_sourceFile, '/');
	if (ptr) return ptr + 1;
	return a_sourceFile;
}
...
static	const char	*ownerString(const char *a_sourceFile, const unsigned int a_sourceLine, const char *a_sourceFunc)
{
	static	char	str[90];
	memset(str, 0, sizeof(str));
	//vsprintf_s(str, "%s(%05d)::%s", sourceFileStripper(sourceFile), sourceLine, sourceFunc);
	sprintf_s(str, sizeof(str), "%s(%05d)::%s", sourceFileStripper(a_sourceFile), a_sourceLine, a_sourceFunc);
...
static	const char	*memorySizeString(unsigned long size)
{
	...
	/*
	if (size > (1024*1024))	vsprintf_s(str, "%10s (%7.2fM)", insertCommas(size), static_cast<float>(size) / (1024.0f * 1024.0f));
	else if (size > 1024)	vsprintf_s(str, "%10s (%7.2fK)", insertCommas(size), static_cast<float>(size) / 1024.0f);
	else					vsprintf_s(str, "%10s bytes     ", insertCommas(size));
	*/

	if (size > (1024 * 1024))	sprintf_s(str, sizeof(str), "%10s (%7.2fM)", insertCommas(size), static_cast<float>(size) / (1024.0f * 1024.0f));
	else if (size > 1024)		sprintf_s(str, sizeof(str), "%10s (%7.2fK)", insertCommas(size), static_cast<float>(size) / 1024.0f);
	else						sprintf_s(str, sizeof(str), "%10s bytes     ", insertCommas(size));
	return str;
}
...
static	sAllocUnit	*findAllocUnit(const void *reportedAddress)
{
	...
	// very good coverage.
	unsigned int	hashIndex = (reinterpret_cast<uintptr_t>(const_cast<void *>(reportedAddress)) >> 4) & (hashSize - 1);
...
static	void	dumpAllocations(FILE *fp)
{
	...
	for (unsigned int i = 0; i < hashSize; i++)
	{
		...
		while(ptr)
		{
			fprintf(fp, "%06d 0x%I64X 0x%I64X 0x%I64X 0x%08X 0x%08X %-8s    %c       %c    %s\r\n",
				...
				//reinterpret_cast<uintptr_t>(ptr->reportedAddress), ptr->reportedSize,
				//reinterpret_cast<uintptr_t>(ptr->actualAddress), ptr->actualSize,
				reinterpret_cast<uint64_t>(ptr->reportedAddress), ptr->reportedSize,
				reinterpret_cast<uint64_t>(ptr->actualAddress), ptr->actualSize,
				m_calcUnused(ptr),
...
static	void	dumpLeakReport()
{
	...
	fprintf(fp, " ---------------------------------------------------------------------------------------------------------------------------------- \r\n");
	fprintf(fp, "|                                          Memory leak report for:																	|\r\n");
	fprintf(fp, " ---------------------------------------------------------------------------------------------------------------------------------- \r\n");
	fprintf(fp, "\r\n");
	fprintf(fp, "\r\n");
	if (s_stats.totalAllocUnitCount)
	{
		fprintf(fp, "%d memory leak%s found:\r\n", s_stats.totalAllocUnitCount, s_stats.totalAllocUnitCount == 1 ? "":"s");
	}
	else
	{
...
public:
	...
	~MemStaticTimeTracker() {s_staticDeinitTime = true; dumpLeakReport();}
};
...
{
	s_breakOnAllocationCount = count;
}
...
void	m_setOwner(const char *file, const unsigned int line, const char *func)
{
	...
	if (s_sourceLine && alwaysLogAll)
	{
		log("[I] NOTE! Possible destructor chain: previous owner is %s", ownerString(s_sourceFile, s_sourceLine, s_sourceFunc));
	}
...
void	*operator new(size_t reportedSize)
{
	...
	const	char		*file = s_sourceFile;
	const	unsigned int	line = s_sourceLine;
	const	char		*func = s_sourceFunc;
...
void	operator delete(void *reportedAddress)
{
	...
	if (reportedAddress) m_deallocator(s_sourceFile, s_sourceLine, s_sourceFunc, m_alloc_delete, reportedAddress);
	else if (alwaysLogAll) log("[-] ----- %8s of NULL                      by %s", allocationTypes[m_alloc_delete], ownerString(s_sourceFile, s_sourceLine, s_sourceFunc));
...
void	operator delete[](void *reportedAddress)
{
	...
	if (reportedAddress) m_deallocator(s_sourceFile, s_sourceLine, s_sourceFunc, m_alloc_delete_array, reportedAddress);
	else if (alwaysLogAll)
		log("[-] ----- %8s of NULL                      by %s", allocationTypes[m_alloc_delete_array], ownerString(s_sourceFile, s_sourceLine, s_sourceFunc));
...
void	*m_allocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int allocationType, const size_t reportedSize)
{
	try
	{
		...
		if (alwaysLogAll) log("[+] %05d %8s of size 0x%08X(%08d) by %s", s_currentAllocationCount, allocationTypes[allocationType], reportedSize, reportedSize, ownerString(sourceFile, sourceLine, sourceFunc));

		// If you hit this assert, you requested a breakpoint on a specific allocation count
		m_assert(s_currentAllocationCount != s_breakOnAllocationCount);
		...
		unsigned int	hashIndex = (reinterpret_cast<uintptr_t>(au->reportedAddress) >> 4) & (hashSize - 1);
		...
		s_stats.totalReportedMemory += static_cast<unsigned int>(au->reportedSize);
		s_stats.totalActualMemory   += static_cast<unsigned int>(au->actualSize);
		s_stats.totalAllocUnitCount++;
		if (s_stats.totalReportedMemory > s_stats.peakReportedMemory) s_stats.peakReportedMemory = s_stats.totalReportedMemory;
		if (s_stats.totalActualMemory   > s_stats.peakActualMemory)   s_stats.peakActualMemory   = s_stats.totalActualMemory;
		if (s_stats.totalAllocUnitCount > s_stats.peakAllocUnitCount) s_stats.peakAllocUnitCount = s_stats.totalAllocUnitCount;
		s_stats.accumulatedReportedMemory += static_cast<unsigned int>(au->reportedSize);
		s_stats.accumulatedActualMemory += static_cast<unsigned int>(au->actualSize);
...
void	*m_reallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int reallocationType, const size_t reportedSize, void *reportedAddress)
{
	try
	{
		...
		if (alwaysLogAll) log("[~] %05d %8s of size 0x%08X(%08d) by %s", s_currentAllocationCount, allocationTypes[reallocationType], reportedSize, reportedSize, ownerString(sourceFile, sourceLine, sourceFunc));
		...
		if (oldReportedAddress != au->reportedAddress)
		{
...

			{
				...
				unsigned int	hashIndex = (reinterpret_cast<uintptr_t>(oldReportedAddress) >> 4) & (hashSize - 1);
		...
		{
			s_stats.accumulatedReportedMemory += deltaReportedSize;
			s_stats.accumulatedActualMemory += deltaReportedSize;
		}
...
void	m_deallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int deallocationType, const void *reportedAddress)
{
	try
	{
		...
		if (alwaysLogAll) log("[-] ----- %8s of addr 0x%08X           by %s", allocationTypes[deallocationType], reinterpret_cast<uintptr_t>(const_cast<void *>(reportedAddress)), ownerString(sourceFile, sourceLine, sourceFunc));
		...
		if (s_staticDeinitTime) dumpLeakReport();
	}
	catch(const char *err)
	{
		// Deal with errors
...
bool	m_validateAllAllocUnits()
{
	...
	// assertion.
	m_assert(allocCount == s_stats.totalAllocUnitCount);
...
void	m_dumpMemoryReport(const char *filename, const bool overwrite)
{
	...
	fprintf(fp, "           Memory tracking overhead: %s\r\n", memorySizeString(s_stats.totalActualMemory - s_stats.totalReportedMemory));
	fprintf(fp, "\r\n");
	...
	fprintf(fp, "           Memory tracking overhead: %s\r\n", memorySizeString(s_stats.peakActualMemory - s_stats.peakReportedMemory));
	fprintf(fp, "\r\n");
